...
from local_session var LocalSession = require('LocalSession');
...
class RemoteSession(asyncore.dispatcher)) {
    ...
    function __init__(self, client_id, rhost, rport, lhost, lport, map) {
    ...
    function handle_close(self) {
        ...
        self.clear();
        MOLO_CLIENT_APP.local_session_dict.pop(id(self), null);
    ...
    function on_start_proxy(self, jdata) {
        /* Handle Start Proxy. */
        LOGGER.debug('on_start_proxy %s', String(jdata));
    ...
    function process_tranparency_pack(self) {
    ...
    function process_molo_tcp_pack(self) {
        ...
        if (ret && self.molo_tcp_pack.error_code == MoloTcpPack.ERR_OK) {
            ...
            LOGGER.debug('RemoteSession process_molo_tcp_pack body:%s', String(;
