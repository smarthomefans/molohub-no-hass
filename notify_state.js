...
try {
    ...
    from urllib.parse var quote = require('quote');
} catch ( ImportError) {
    quote = urllib.pathname2url;
}
from const var (BIND_AUTH_STR_TEMPLATE_DEFAULT,
 = require('(BIND_AUTH_STR_TEMPLATE_DEFAULT,
');
...
class NotifyState) {
    ...
    function update_state(self, data) {
        ...
        LOGGER.debug('cur_data %s', String(self.cur_data));
    ...
    function generate_str_serverconnected(self) {
        ...
        while (i < opentype_count) {
            token_list.push(self.molo_server_host_str);
    ...
    function generate_str_auth_binded(self) {
        ...
        if (opentype !in self.cur_data.get('platform_icon')) {
            opentype = 'default';
